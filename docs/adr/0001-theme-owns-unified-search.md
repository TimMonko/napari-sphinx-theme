...
- **Old Sphinx search must be neutralized** on opted-in sites (override
  `search-button-field`/`search-field`, drop `#pst-search-dialog` and the Ctrl+K
  handler), or users hit two different searches.
- **Merge-list network effect.** A dead sibling bundle can fail the whole search;
  the installer must catch failed merges *per index* so non-opted-in or broken
  sites are dropped rather than fatal.
